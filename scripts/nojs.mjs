...
// are kept; the script exits 1 if any executable script survives. The .br/.gz sidecars are
// not copied: they still hold the un-stripped document, so a sidecar-negotiating server
// would serve the scripts back and turn this audit target into a lie.
...
cpSync(dist, out, { recursive: true, filter: (src) => !/\.(br|gz)$/i.test(src) });
...
const sidecars = globSync('**/*.{br,gz}', { cwd: out });
console.log(`[nojs] ${files.length} html file(s) written to dist-nojs/; executable scripts left: ${left.length ? left.join(', ') : 'none'}; compressed sidecars left: ${sidecars.length ? sidecars.join(', ') : 'none'}`);
process.exitCode = left.length || sidecars.length ? 1 : 0;
